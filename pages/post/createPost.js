						...
						<div className="newPost__content__main__metaInpust">
							...
							<label htmlFor="upload" style={{ cursor: 'pointer', width: '24%', margin: '10px 0px' }}>
								...
								>
									...
									<PublishIcon />
									<span
										style={{ marginLeft: '1px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}
									>
							...
							>
								...
								>
									{postTagsFilled && isAutoCompleteOpen
										? filtredTags.map((tag) => {
												return (
													<li onClick={ACItemClickHandler} className="tag__autocomplete__item" key={tag.id}>
														{tag.value}
													</li>
												)
										  })
